...
class CineLinkService extends gen_server_1.CineLinkServiceBase {
    ...
    async setMedia(request, client) {
        ...
        const mediaUrl = normalizeUrl(request.url);
        if (!mediaUrl) {
    ...
    removeParticipantAndReassignHost(state, userId) {
        ...
        return { changed: participantChanged, action };
    }
    ...
    async addToQueue(request, client, position) {
        ...
        if (position === "next") {
            ...
            return this.acceptWithReason("Added right after current video.", state);
        }
        ...
        return this.acceptWithReason("Added to queue end.", state);
    }
...
async function validatePlayableMediaUrl(url) {
    try {
        ...
        if (contentType.includes("text/html")) {
            return "Google Drive link is not a direct video stream. Make the file public and use a direct media URL.";
        }
        return undefined;
    }
    catch (error) {
