...
test("playback controls with Plyr engine", async ({ page, request }, testInfo) => {
  ...
  try {
    ...
    const recentLogs = await request.get(`${testApiBase}/test/logs/recent?limit=200`);
    const logsBody = await recentLogs.json().catch(() => ({}));
    await testInfo.attach("browser-console.json", {
      body: JSON.stringify(browserConsole, null, 2),
      contentType: "application/json"
    });
    await testInfo.attach("server-test-logs.json", {
      body: JSON.stringify(logsBody, null, 2),
      contentType: "application/json"
    });
  }
});
